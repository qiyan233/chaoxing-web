...
from api.session_context import SessionContext, CancelledError


MULTIMEDIA_LOG_URL = "https://mooc1.chaoxing.com/mooc-ans/multimedia/log/a"
...
class SessionManager:
    ...
    def get_session(cls) -> requests.Session:
        # 优先返回当前线程绑定的隔离会话（Web 模式）
        holder = SessionContext.get_session_holder()
        if holder is not None:
            return holder.session
        # 未绑定隔离会话时回退到进程内单例，不做 cookies 文件持久化
...
class Chaoxing:
    ...
    def _check_cancelled(self):
        """检查 Web 后台任务取消信号；未绑定时无副作用"""
        if SessionContext.is_cancelled():
            raise CancelledError("任务已被取消")

    def _emit(self, event_type: str, **payload):
        """上报进度事件到 Web 端 SSE；未绑定时无副作用"""
        SessionContext.report_progress(event_type, **payload)
    ...
    def video_progress_log(
        ...
        # 超星通过这个接口记录视频/音频进度；返回 isPassed=True 才表示任务点完成。
        _url = f"{MULTIMEDIA_LOG_URL}/{_course['cpi']}/{_dtoken}"
        ...
        if rt:
            ...
            for rt in [0.9, 1]:
                ...
                if resp.status_code == 200:
                    ...
                    result = resp.json()
                    is_passed = bool(result.get("isPassed"))
                    logger.info(
                        "视频进度上报: jobid={}, playingTime={}/{}, rt={}, isPassed={}",
                        _job.get("jobid"),
                        _playingTime,
                        _duration,
                        rt,
        ...
        if resp.status_code == 200:
            ...
            is_passed = bool(result.get("isPassed"))
            logger.info(
                "视频进度上报: jobid={}, playingTime={}/{}, isPassed={}",
                _job.get("jobid"),
                _playingTime,
                _duration,
                is_passed,
    ...
    def study_video(self, _course, _job, _job_info, _speed: float = 1.0, _type: Literal["Video", "Audio"] = "Video") -> StudyResult:
        ...
        self._emit("job_done", job_id=_job.get("jobid"), job_name=_job.get("name"), kind="video")
        return StudyResult.SUCCESS
